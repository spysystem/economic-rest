# EconomicRest\VatAccountsApi

All URIs are relative to https://restapi.e-conomic.com, except if the operation defines another base path.

| Method | HTTP request | Description |
| ------------- | ------------- | ------------- |
| [**getVatAccount()**](VatAccountsApi.md#getVatAccount) | **GET** /vat-accounts/{id} |  |


## `getVatAccount()`

```php
getVatAccount($id): \EconomicRest\Model\VatAccount
```
...
$apiInstance = new EconomicRest\Api\VatAccountsApi(
...
try {
    $result = $apiInstance->getVatAccount($id);
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling VatAccountsApi->getVatAccount: ', $e->getMessage(), PHP_EOL;
}
```
...
[**\EconomicRest\Model\VatAccount**](../Model/VatAccount.md)
