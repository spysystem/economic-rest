...
/**
 * VatAccountApi Class Doc Comment
 *
 * @category Class
 * @package  EconomicRest
 * @author   OpenAPI Generator team
...
class VatAccountApi
{
    ...
    public function searchVatAccountsWithHttpInfo($skip_pages = null, $page_size = 500, string $contentType = self::contentTypes['searchVatAccounts'][0])
    {
        ...
        try {
            ...
            switch($statusCode) {
                case 200:
                    if ('\EconomicRest\Model\SearchVatAccountsResponse' === '\SplFileObject') {
                        $content = $response->getBody(); //stream goes to serializer
                    } else {
                        $content = (string) $response->getBody();
                        if ('\EconomicRest\Model\SearchVatAccountsResponse' !== 'string') {
                            try {
                    ...
                    return [
                        ObjectSerializer::deserialize($content, '\EconomicRest\Model\SearchVatAccountsResponse', []),
            ...
            switch ($e->getCode()) {
                case 200:
                    $data = ObjectSerializer::deserialize(
                        $e->getResponseBody(),
                        '\EconomicRest\Model\SearchVatAccountsResponse',
    ...
    public function searchVatAccountsRequest($skip_pages = null, $page_size = 500, string $contentType = self::contentTypes['searchVatAccounts'][0])
    {
        ...
        if ($page_size !== null && $page_size > 1000) {
            throw new \InvalidArgumentException('invalid value for "$page_size" when calling VatAccountApi.searchVatAccounts, must be smaller than or equal to 1000.');
        }
        if ($page_size !== null && $page_size < 1) {
            throw new \InvalidArgumentException('invalid value for "$page_size" when calling VatAccountApi.searchVatAccounts, must be bigger than or equal to 1.');
        }
        

        $resourcePath = '/vat-accounts';
        $formParams = [];
        $queryParams = [];
        $headerParams = [];
        $httpBody = '';
